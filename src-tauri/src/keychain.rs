...
// entry per provider, namespaced under the app's bundle identifier so the
// items are app-private (no keychain-access-groups entitlement needed under the
// App Sandbox). All the persistence policy — what to store, migration, the
// on-disk fallback — lives on the TypeScript side (see stores/secret-config.ts);
// these commands are the thin transport.
