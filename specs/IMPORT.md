...
- **Account mapping** lives in a Map-accounts dialog the table's ACCOUNT cells open — one row per imported account, each mapping to an existing budget account or marked for creation (the default); edits apply live. The ACCOUNT cell shows only the mapped *target* name, with a "new" tag when merge would create the account — the raw imported → target relationship is visible in the dialog. The merge dialog is a read-only safety gate: above the row count and sum, a per-account summary lists every destination account — its source account name(s), a "new" badge when merge creates it, the transaction count, and the resulting balance — so a wrong account mapping (including a mis-pointed transfer target) is visible before commit. The summary groups by the *resulting* account, so a transfer's counterpart surfaces even when no import source names it. Beyond five accounts the list scrolls, capped so the dialog's Cancel/Merge buttons always stay on-screen. Aliases stored in `.capy/aliases.json` survive across imports and pre-populate the mapping; they're persisted at merge.
