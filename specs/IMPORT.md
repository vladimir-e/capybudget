...
Both paths feed one builder — `buildStaged(records) → ImportTransaction[]` — that owns id assignment (sequential `imp-N`, continuing across files), the trim-45 `description`, sign/type normalization, date coercion (any supported format → `YYYY-MM-DD`; an unparseable date — a "Pending" column, a cropped screenshot — falls back to today, since staging read-back drops non-ISO dates), and empty resolved fields. One staging format, one set of invariants.
