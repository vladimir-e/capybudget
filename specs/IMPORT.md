...
- **Account mapping** is confirmed in the merge dialog — one row per imported account, each mapping to an existing budget account or marked for creation (the default), editable at the moment it takes effect. The table's ACCOUNT column shows the mapped *target* name (the imported string as a secondary line when they differ) with a "new" tag when merge would create the account. Aliases stored in `.capy/aliases.json` survive across imports and pre-populate the mapping; they're persisted at merge.
