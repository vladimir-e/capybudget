...
- **Account mapping** lives in a set of mapping rows — one per imported account, each mapping to an existing budget account or marked for creation (the default), edits applying live. The same rows render in two places: the Map-accounts dialog the table's ACCOUNT cells open, and the merge confirmation. The ACCOUNT cell shows only the mapped *target* name, with a "new" tag when merge would create the account — the raw imported → target relationship is visible in the rows. The merge dialog is an editable safety gate: each source-account row shows its destination's resulting balance, a "new" badge when merge creates it, and the row count, so a wrong mapping is both visible and correctable before commit — fixing it there flows into the merge and live-recomputes the balances. Rows are the account-mapping axis only; a transfer's counterpart is not a mapping and gets no row, but its leg still counts toward the destination balance. **Transfers that lose their counterpart** — the merge couldn't pair them, so they import as income/expense — collapse into a single amber warning line with their count, never enumerated. Beyond five rows the list scrolls (the selector's dropdown portals out so it isn't clipped), capped so the dialog's Cancel/Merge buttons always stay on-screen. Aliases stored in `.capy/aliases.json` survive across imports and pre-populate the mapping; they're persisted at merge.
