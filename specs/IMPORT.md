...
2. **Categorizing batch** — ~25 rows + their pre-attached history context in → `{ id, merchant, category, confidence }[]` out, where `category` is a budget category *name* (the model reasons over names, never ids; code maps the name back to a `categoryId`, matching exactly first, then tolerating the `Name (Group)` form the prompt's own category list displays, then retrying with a final parenthetical stripped so a wrong or misspelled group still lands — a name that resolves to nothing leaves the row uncategorized and re-enrichable). Batches are independent, run bounded-parallel, and fail in isolation.
