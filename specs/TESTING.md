...
Journey tests exercise real user interactions (open form, type, click, submit) against the complete component tree including routing, TanStack Query, mutations, and undo/redo. Each file pays a one-time cold-start to transform and import the full app module graph, so they run as a dedicated `full-app` Vitest project (`vite.config.ts`) with `pool: "threads"` and `isolate: false` — sharing the module graph within the worker keeps the suite fast. The partition is by **cost-class, not directory**: any test that mounts the whole app via `renderApp()` belongs to this project, including ones colocated beside their component (e.g. `components/budget/first-run-guide.test.tsx`, `budget-selector.test.tsx`) rather than under `journeys/`. The list lives in `fullAppMountTests` in `vite.config.ts`. The rest of the suite runs in the `unit` project on defaults; `isolate: false` is scoped to `full-app` because it causes cross-file state bleed elsewhere — files in this project must reset their own state per test (`renderApp`'s `afterEach`, or a `beforeEach` mock/store reset). A 30s per-test timeout gives headroom over the cold-start under CPU contention.
