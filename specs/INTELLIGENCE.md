...
- **Handlers** — per-tool implementations:
  ...
  - **CSV tools** — `analyze_csv`, `preview_transform`, `transform_csv`, `auto_enrich`, `apply_account_aliases`, `enrich_status` (stats; `sampleSize > 0` also returns sample rows), `enrich_update`
...
`auto_enrich` and `apply_account_aliases` are **code-triggered, never advertised**: neither has a `TOOL_MODES` entry, so no model sees them, but they stay on the MCP surface and dispatchable via `runTool` — the import orchestrator runs each as a deterministic phase pre-step (`apply_account_aliases` before the accounts turn, `auto_enrich` before the enrich turn). Dispatch (`runTool`) does not consult `TOOL_MODES`; the map gates *advertisement* only, so removing an entry never breaks a direct call.
