...
- **Halt path.** If every staging row is a duplicate after auto-mark + review (the whole import overlaps what's in the budget), the model stops without proceeding to enrich and reports it — a terminal result, "already in your budget — nothing to import" (the `NOTHING_TO_IMPORT_SIGNAL` token marks the seam for Unit 5's prominent terminal rendering).
